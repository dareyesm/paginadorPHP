...
class Books{
    ...
    public function show_books($init = FALSE, $page_size = FALSE){
        
        $sth = $this->objdata->prepare('SELECT * FROM books LIMIT :init, :size');
        $sth->bindParam(':init', $init, PDO::PARAM_INT);
        $sth->bindParam(':size', $page_size, PDO::PARAM_INT);
    ...
    public function counting_books(){
        $sth = $this->objdata->prepare('SELECT * FROM books');
        $sth->execute();
        return $qty = $sth->rowCount();
    }
}
